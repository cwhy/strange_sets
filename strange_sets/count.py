...
class CountSquares(BaseDataSet):
    ...
    def _get_one(self):
        while True:
            ...
            dots_ctr_0 = np.random.randint(self.dim_X[0], size=self.n_trials)
            dots_ctr_1 = np.random.randint(self.dim_X[1], size=self.n_trials)
            dot_size = np.random.randint(1, self.max_size, size=self.n_trials)
            dots_area_lists = ([], [])
            n_dots = 0
